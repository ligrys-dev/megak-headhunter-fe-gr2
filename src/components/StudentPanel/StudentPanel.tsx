...
import {getStudentProfile} from "../../api/get-student-profile";
...
export const StudentPanel = () => {
    ...
    return (
        <div className="student_panel">
            <HeaderPanel name={user?.profile?.firstName} lastName={user?.profile?.lastName} urlAccount="/student"
                         avatar={user?.profile?.githubUsername} onChildClick={handleChildHeaderClick}/>
            <div className="panel_main">
                <BookmarksPanel user={user} bookmarks={bookmarks} bookmarksView={bookmarksView} onChildClick={handleChildBookmarksClick}/>
                {bookmarksView ? '' : (password ? <ChangePassword/> : (user ? <WelcomeView name={user?.profile?.firstName}/> : <EditStudentData onSubmit={handleCreateStudentProfile}/>))}
            </div>
        </div>
    );
};
